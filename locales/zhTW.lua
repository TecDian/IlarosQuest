...
L["EQ_PURGETXT"] = "你確定要刪除所有舊資料?如果你不確定，或者剛剛才完成升級，取消後並檢查IlarosQuest視窗看看是否有你的任務。要繼續的話 在下面對話框中輸入'DELETE'，此動作是無法恢復的。你也可以在清除資料庫後使用介面選項。"
-- L["DELETE"] = ""
L["EQ_UPGRADETXT"] = "IlarosQuest從資料庫中(或從升級版本中)檢測到 %s 筆毀損的資料，點擊'是'來嘗試升級看看。這需要一點時間。升級後你可以使用介面選項。"
L["Failed"] = "已失敗"
...
L["WE"] = "節日"
L["Y"] = "每日"
L["You are running a source checkout of IlarosQuest %s directly from the repository."] = "你正在執行的IlarosQuest %s是從儲存庫檢查比對出來的。"
L["You are running an ILAROS RELEASE of IlarosQuest %s (revision %d). If you'd like to improve it, contact the team!"] = "你正在運行一個ILAROS RELEASE指令為IlarosQuest %s (修正 %d)."
L["r%d"] = "r%d"
...
L["IlarosQuest Log"] = "IlarosQuest紀錄"
L["Filters"] = "篩選"
L["Options"] = "選項"
...
|cffFFAFAF備註:要匯入所有資料的話你必須在所有角色上重複此功能，且還需要重新載入插件。這是IlarosQuest儲存每個角色資料的限制 (這可節省你的記憶體/啟動時間)。|r]=]
...
L["EQ_DESC"] = "IlarosQuest 允許你在任務清單中追蹤你已完成的任務，在每個地圖中也顯示可接到哪些任務。結合LightHeaded和TomTom使用本插件即可獲得博學大師的稱號"
L["EQ_LOCALLIST"] = "警告:本地化任務清單標題可能會導致斷線。這是由於伺服器查詢一個任務的提示訊息或者是因為伺服器端沒有該任務的相關訊息。"
L["Enable Filtering"] = "啟用篩選"
L["Example for Ignored quest: Some quests are still in the database but are not attainable ingame anymore. Ignore them to hide them from the list."] = "為忽略任務舉例:有些任務仍然存在於的資料庫但並不存在於遊戲中。故可從清單中忽略並隱藏它們。"
...
L["Track Zone"] = "追蹤地區"
L["Track zone changes and update the IlarosQuest list for that zone"] = "追蹤變換地區時同時更新該地區的IlarosQuest清單"
...
需經過兩個步驟，IlarosQuest可以讓你從Blizzard伺服器中取得你已完成任務的歷史紀錄! 找到那些你錯過的任務然後再使用IlarosQuest。
...
如果你的聊天視窗中出現一條紅色訊息，表示IlarosQuest的資料庫中不包含一些你已完成的任務。不過不必擔心，Blizzard的日誌中包含一些特殊的任務，因此你還沒有轉移到任何一個。例如 |cffFFFF00[旗幟:聯賽邀請]|r]=]
