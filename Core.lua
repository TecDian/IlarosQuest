...
local addon = LibStub("AceAddon-3.0"):NewAddon("IlarosQuest", "AceConsole-3.0", "AceEvent-3.0", "AceHook-3.0")
IlarosQuest = addon
...
do -- Borrowed from BigWigs :)
	...
	local majorVersion = GetAddOnMetadata("IlarosQuest", "Version") or "2.?"
	if releaseRevision == -1 then
		releaseString = L["You are running a source checkout of IlarosQuest %s directly from the repository."]:format(majorVersion) -- Version message, %s = majorVersion
	elseif releaseType == RELEASE then
		--releaseString = L["You are running an official release of IlarosQuest %s (revision %d)"]:format(majorVersion, releaseRevision) -- Version message, %s = majorVersion, %d = revision
	elseif releaseType == ALPHA then
		releaseString = L["You are running an ILAROS RELEASE of IlarosQuest %s (revision %d). If you'd like to improve it, contact the team!"]:format(majorVersion, releaseRevision) -- Version message, %s = majorVersion, %d = revision
	end
	_G.ILAROSQUEST_RELEASE_TYPE = releaseType
	_G.ILAROSQUEST_RELEASE_REVISION = releaseRevision
	_G.ILAROSQUEST_RELEASE_STRING = releaseString
...
--[[StaticPopupDialogs["ILAROSQUEST_UPGRADEDB"] = {
  ...
  OnAccept = function()
	  addon:Debug("ILAROSQUEST_UPGRADEDB: OnAccept")
	  addon:UpgradeDB()
  end,
  OnCancel = function()
...
StaticPopupDialogs["ILAROSQUEST_PURGEOLD"] = {
	...
	OnAccept = function()
		addon:Debug("ILAROSQUEST_PURGEOLD: OnAccept")
...
function addon:OnInitialize()
    ...
    self.db = LibStub("AceDB-3.0"):New("IQ2DB", {
			profile = {
				debug = false,
				localized = {
					["*"] = {},
	...
	db = self.db.profile
	self.dbpc = LibStub("AceDB-3.0"):New("IQ2DBPC", {
			profile = {
		        zoneid = nil,
				dbversion = 3,
				zonegroup = nil,
	...
	IlarosQuest:SecureHook(GameTooltip, "SetHyperlink", "SetHyperlink")
end
...
do
	local info = {}
	local IlarosQuest_ZoneMenu = CreateFrame("Frame", "IlarosQuest_ZoneMenu")
	IlarosQuest_ZoneMenu.displayMode = "MENU"
	IlarosQuest_ZoneMenu.point = "TOPRIGHT"
	--/run DropDownList1:SetPoint("TOPRIGHT", "EQ_MenuButton", "BOTTOMRIGHT",0,0)
	IlarosQuest_ZoneMenu.relativeTo = "EQ_MenuButton"
	IlarosQuest_ZoneMenu.relativePoint = "BOTTOMRIGHT"
	IlarosQuest_ZoneMenu.initialize = function(self, level)
...
do
	...
	local IlarosQuest_QuestMenu = CreateFrame("Frame", "IlarosQuest_QuestMenu")
	IlarosQuest_QuestMenu.displayMode = "MENU"
	IlarosQuest_QuestMenu.initialize = function(self, level)
		if not level then return end
...
function addon:Toggle()
	if IlarosQuestFrame:IsShown() then
		IlarosQuestFrame:Hide()
	else
		IlarosQuestFrame:Show()	
		self:UpdateFrame()
	end
end
function addon:OnShow()
	if dbpc.posx and dbpc.posy then
		IlarosQuestFrame:ClearAllPoints()
		IlarosQuestFrame:SetPoint("TOPLEFT","UIParent", "BOTTOMLEFT", dbpc.posx, dbpc.posy)
	end
end
function addon:SavePosition()
	local Left = IlarosQuestFrame:GetLeft()
	local Top = IlarosQuestFrame:GetTop()
...
--[[function addon:UpgradeCheck()
	---------------------------------------------------------------------
	--- Handle Migration
	---	IlarosQuestDBPC = dbversion 1
	---	IQ2DBPC.QuestHistory = dbversion 2
	--- IQ2DBPC.history = dbversion 3
	if IlarosQuestDBPC ~= nil then
		for profilename,profile in pairs(IlarosQuestDBPC) do 
			...
			if profile.QuestHistory ~= nil and not profile.upgraded then
				...
				--StaticPopupDialogs["ILAROSQUEST_UPGRADEDB"].text = string.format(L["EQ_UPGRADETXT"], oldprofilecount)
				--StaticPopup_Show ("ILAROSQUEST_UPGRADEDB")
...
function addon:UpgradeDB()
	...
	dbpc.dbversion = 2
	LibStub("AceConfigRegistry-3.0"):NotifyChange("IlarosQuest")
end]]
...
function addon:ShowConfig()
	...
	InterfaceOptionsFrame_OpenToCategory(self.optionsFrames.IlarosQuest)
end
...
function addon:AddUnknown(uid, category, title)
	if uid and uid ~= nil then
		...
		if db.unknowns[uid] == nil then
			db.unknowns[uid] = ILAROSQUEST_RELEASE_REVISION .. "||" .. date("%x") .. "||" .. tostring(title) .. "||" .. tostring(category)
		end
	end
end
...
function addon:UpdateFrame(zid, zgid)
	...
	if IlarosQuestFrame:IsShown() then
		...
		FauxScrollFrame_Update(IlarosQuestListScrollFrame,questcount,27,16)
		local scrolloffset = FauxScrollFrame_GetOffset(IlarosQuestListScrollFrame)
		local quest
		if questcount then
			if questlist then
				for k, quest in pairs (questlist) do
		...
		for j = buttonid, 27 ,1 do
			button = getglobal("IlarosQuestTitle"..j)
			button:Hide()
		end
...
-- @usage questhistory = IlarosQuest:GetHistory(uid [, zoneid])
...
-- @name IlarosQuest:GetHistoryStatus
-- @param uid Integer > 1: Unique Quest ID
-- @param zoneid Integer: Numerical quest category
...
-- nil: Unknown quest
-- @usage questhistory = IlarosQuest:GetHistoryStatus(uid [, zoneid])
...
-- @return zoneid
-- @usage 
-- uid, quest, zoneid = IlarosQuest:GetQuestData(uid, nil, nil, zoneid)
-- uid, quest, zoneid = IlarosQuest:GetQuestData(uid)
-- uid, quest, zoneid = IlarosQuest:GetQuestData(nil, category, questname)
-- uid, quest, zoneid = IlarosQuest:GetQuestData(uid, category, nil, zoneid)
...
local que = {}
local doneList = {}
local tt = CreateFrame("GameTooltip", "IlarosQuestScanningTooltip", UIParent, "GameTooltipTemplate")
local ttlt = IlarosQuestScanningTooltipTextLeft1
...
function addon:UpdateButtonTextByUID(uid)
	for k,v in pairs(questdisplay) do
		if v.id == uid then
			...
			local button = getglobal("IlarosQuestTitle"..k)
			local text = "["
...
function addon:UpdateButton(buttonid, quest, uid)
	--if db.view == "history" or db.view == "zone" then
		local button = getglobal("IlarosQuestTitle"..buttonid)
		local qTag = ""
		local strLM = ""
...
function addon:ButtonClick(self, button)
	...
	if button == "LeftButton" then
		if ( IsModifiedClick() ) then
			...
			if IsAddOnLoaded("Lightheaded") then
				...
				QuestLogFrame:SetPoint("TOPLEFT",IlarosQuestFrame, "TOPLEFT", 360, 0)
			end
	...
	elseif button == "RightButton" then
		-- open quest menu
		ToggleDropDownMenu(1, nil, IlarosQuest_QuestMenu, self:GetName(), 0, 0)
	end
end
...
function addon:LoadQuestData(group)
	if group ~= nil then
		...
		return IlarosQuestData[group] --questdata[varname] and varname
	else
...
function IlarosQuest_ScrollFrame_Update()
	addon:UpdateFrame()
end
