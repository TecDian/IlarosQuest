	...
	local function iter(t, prestate)
		...
		while state do
			if value then
				...
				local displaynpc = true
				local icon = "Interface\\AddOns\\IlarosQuest\\images\\normal-quest"
				local showcount = tonumber(qcount)
				for _,uid in pairs(quests2) do
					side = 3
					showquest = false
					...
					if daily ~= nil then
						icon = "Interface\\AddOns\\IlarosQuest\\images\\daily-quest"
					end
...
function QG:OnInitialize()
	...
	IlarosNote:RegisterPluginDB(L["Quest Givers"], HTHandler, options)
end
