...
-- Release-String mit Versionsnummer
do
    local ReleaseVersion = GetAddOnMetadata("IlarosQuest", "Version")
    releaseString = L["version %s."]:format(ReleaseVersion)
    _G.ILAROSQUEST_RELEASE_VERSION = ReleaseVersion
end
...
-- Nur für Konfigurationsfenster verwendet.
-- Für Tooltips sattdessen addon:GetColor(hex), obwohl die Konstanten drinstehen. Was ist da los?
-- Für Questliste stattdessen IlarosQuest.xml
EQ_TextColors["s-1"] = { r = 1.00, g = 0.00, b = 0.00, font = EQFont_Failed, hex = "ff0000" };
EQ_TextColors["s0"] = { r = 0.01, g = 0.53, b = 0.93, font = EQFont_Progress, hex = "0286ec" };
EQ_TextColors["s1"] = { r = 1.00, g = 0.75, b = 0.00, font = EQFont_Complete, hex = "ffbf00" };
EQ_TextColors["s2"] = { r = 0.0, g = 1.00, b = 0.00, font = EQFont_TurnedIn, hex = "00ff00" };
...
function addon:OnInitialize()
    ...
    self.db = LibStub("AceDB-3.0"):New("QuestMarks", {
            profile = {
                debug = false,
                localized = {
                    ["*"] = {},
    ...
    db = self.db.profile
    self.dbpc = LibStub("AceDB-3.0"):New("QuestConfig", {
            profile = {
                zoneid = nil,
                dbversion = 3,
                zonegroup = nil,
...
--[[function addon:UpgradeCheck()
    ...
    --- QuestConfig.QuestHistory = dbversion 2
    --- QuestConfig.history = dbversion 3
...
function addon:AddUnknown(uid, category, title)
    if uid and uid ~= nil then
        ...
        if db.unknowns[uid] == nil then
            db.unknowns[uid] = ILAROSQUEST_RELEASE_VERSION .. "||" .. date("%x") .. "||" .. tostring(title) .. "||" .. tostring(category)
        end
    end
end
