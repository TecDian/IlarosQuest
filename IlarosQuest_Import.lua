...
local Import = IlarosQuest:NewModule(MODNAME)
local db, dbpc
...
function Import:OnInitialize()
    --if (IsAddOnLoaded("QuestHistory") or IsAddOnLoaded("QuestGuru")) then
        ...
        IlarosQuest:RegisterModuleOptions(MODNAME, getOptions, L["Import"]) -- Import Options Header
    --else
        --IlarosQuest:Debug("Import:OnInitialize failed because QuestHistory and/or QuestGuru are not loaded.")
    --end
end

function Import:ImportQuestGuru()
    local count = 0
    --IlarosQuest:Debug("I wanted to import your QuestGuru history, but sadly I haven't gotten around to it yet. Bummer.")
    IlarosQuest:Debug(playername)
    ...
    if (QuestGuru_Quests and QuestGuru_Quests[server]) then
        ...
        IlarosQuest:Print( sfmt(L["Imported %d quests from %s"], count,  -- Import finished message, %d = number imported, %s = localized import source
                                                                        L["QuestGuru"]) ) -- Import source
    else
        IlarosQuest:Print(L["Import failed, you have no data relating to this character on this server."]) -- Import failed message
    end
end

function Import:ImportQuestHistory()
    local count = 0
    --IlarosQuest:Debug("I wanted to import your quest history from QuestHistory, but sadly I haven't gotten around to it yet. Bummer.")
    ...
    if (QuestHistory_List and QuestHistory_List[server] and QuestHistory_List[server][playername]) then
        for k,v in pairs(QuestHistory_List[server][playername]) do
            ...
            if (currentid["lk"]) then -- WOWZERS, IT HAS A LINK!
                ...
                quest, zone = IlarosQuest:GetQuestData(nil, currentid["c"], currentid["t"])
...
function Import:ImportQuest(questid, zoneid, status, from)
    IlarosQuest:Debug("We're trying to import quest "..tostring(questid).. " which is from zone "..tostring(zoneid).. " with a status of " .. tostring(status) .. " from addon " .. tostring(from)..".")
    if (not zoneid) then
        --- we need to try to get the zoneid here for quests without one passed in, super bummer here because it's a pain
        quest, zoneid = IlarosQuest:GetQuestData(questid)
    else
        quest, zoneid = IlarosQuest:GetQuestData(questid, nil, nil, zoneid)
    end
    local b = -2
    ...
    if (cmpstatus(status, b) == 1) then
        ...
        IlarosQuest:Debug("status: " .. tostring(status) .. "  b: " .. tostring(b) .. "  cmp: " .. cmpstatus(status, b))
        IlarosQuest:AddQuest(quest.id, nil, nil, zoneid)
...
function Import:ClearImport()
    ...
    IlarosQuest:Print(string.format(L["Deleted %d quests from your database"], count)) -- Import: Delete imported data message
    clrimportchk = false
end
