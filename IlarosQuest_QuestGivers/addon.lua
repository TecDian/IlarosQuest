...
local QG = IlarosQuest:NewModule(MODNAME, "AceEvent-3.0")
local db, dbpc
local options
...
local function hidePin(button, mapFile, coord)
	...
	-- QG:SendMessage("IlarosNote_NotifyUpdate", "QuestGivers")
end
local function createWaypoint(button, mapFile, coord)
	...
	if IlarosNavi then
		IlarosNavi:AddZWaypoint(c, z, x*100, y*100, npcname)
	elseif Cartographer_Waypoints then
		Cartographer_Waypoints:AddWaypoint(NotePoint:new(IlarosNote:GetCZToZone(c, z), x, y, npcname))
	end
end
...
local HT_Dropdown = CreateFrame("Frame", "IlarosNote_QuestGiversDropdownMenu")
	...
	for i in pairs(displaynpcs) do
		...
		for _,uid in pairs(quests2) do
			...
			--if string.find(uid, "\\%") ~= nil then
				...
				side = tonumber(side)
				-- IlarosQuest:Print("uid: " .. tostring(uid) .. " - side:"..tostring(side))
			--end
			
			if db.LoadData and not InCombatLockdown() then
				_, questData, zone = IlarosQuest:GetQuestData( tonumber(uid) )
			...
			if IlarosQuest.dbpc.profile.ignored[tonumber(uid)] then showquest = false end
			local status, dcount = IlarosQuest:GetHistoryStatus(tonumber(uid))
			if db.LoadData and not InCombatLockdown() then

				-- print(tostring(zone))
				-- print(IlarosQuest:GetCategory(zone) .. "Toggle")
				local togglevar = IlarosQuest:GetCategory(zone)
		...
		-- IlarosQuest:Print("showcount: " .. tostring(showcount))
		del(quests2)
		if showcount > 0 then
			if db.LoadData and db.QuestNames and not InCombatLockdown() then
				tooltip:AddLine( string.format("|cffffff00%s|r (%d)", L[npcname], showcount), 1, 1, 0 )
				for _, str in pairs(questNames) do
				   tooltip:AddLine( "  " .. L[tostring(str)], 1, 1, 1 )
				end
			else
				tooltip:AddLine("|cffffff00"..L[npcname].."|r", 1, 1, 0)
                if showcount > 1 then
				    tooltip:AddLine("  " .. string.format(L["%d quests left"], showcount), 1, 1, 1)
                else
				    tooltip:AddLine("  " .. string.format(L["%d quest left"], showcount), 1, 1, 1)
                end
			end
			hasNPCsShown = true
		end
	...
	local function iter(t, prestate)
		...
		while state do
			if value then
				...
				local displaynpc = true
				local icon = "Interface\\AddOns\\IlarosQuest_QuestGivers\\Artwork\\Quest"
				local showcount = tonumber(qcount)
				for _,uid in pairs(quests2) do
					side = 3
					showquest = false
					--if string.find(uid, "\\%") ~= nil then
						uid, side = strsplit("%", uid)
						side = tonumber(side)
						--IlarosQuest:Print("uid: " .. uid .. " - side:"..side)
					--end
					...
					local status, daily = IlarosQuest:GetHistoryStatus(tonumber(uid))
					if daily ~= nil then
						icon = "Interface\\AddOns\\IlarosQuest_QuestGivers\\Artwork\\DQuest"
					end
					-- if tonumber(npcid) == 20735 then
						-- IlarosQuest:Print("npcid: " .. tostring(npcid) .. " - showquest: " .. tostring(showquest) .. " - uid: " .. tostring(uid) .. " - status: " .. tostring(status) .. " - daily:"..tostring(daily) .. " - side:"..tostring(side))
					-- end
					if db.LoadData and not InCombatLockdown() then
						-- print(tostring(zone))
						-- print(IlarosQuest:GetCategory(zone) .. "Toggle")
				...
				del(quests2)
				-- if tonumber(npcid) == 20735 then
				-- IlarosQuest:Print("npcid: " .. tostring(npcid) .. " - showcount: " .. tostring(showcount) .. " - qcount:"..tostring(qcount))
				-- end
				if showcount > 0 then
					-- local x2, y2 = IlarosNote:getXY(state)
...
local function getOptions()
	if not options then
		options = {
			...
			set = function(info, value) db[ info[#info] ] = value QG:SendMessage("IlarosNote_NotifyUpdate", "QuestGivers") end,
			args = {
				LoadData = {
					type = "group",
					order = 0,
					...
					args = {
						...
						LoadData = {
							order = 10,
							type = 'toggle',
							name = L["Load Data from IlarosQuest database"],
						...
						QuestNames = {
							...
							desc = L["Query the IlarosQuest database for quest names"],
				...
				Faction = {
					...
					set = function(info, value) db.filters[ info[#info] ] = value QG:SendMessage("IlarosNote_NotifyUpdate", "QuestGivers") end,
					args = {
						desc = {
				...
				Category = {
					...
					set = function(info, value) db.filters.Categories[ info[#info] ] = value QG:SendMessage("IlarosNote_NotifyUpdate", "QuestGivers") end,
					disabled = function() return not db.LoadData end,
					args = {
...
function QG:OnInitialize()
	-- Set up our database
	self.db = IlarosQuest.dbpc:RegisterNamespace("QuestGivers", defaults)
	-- dbpc = IlarosQuest.dbpc.profile
	IlarosQuest:RegisterModuleOptions(MODNAME, getOptions, L["Quest Givers"])
	db = self.db.profile

	-- Initialize our database with IlarosNote
	IlarosNote:RegisterPluginDB("QuestGivers", HTHandler, options)
end
